...
- Статус: completed / docs-only closing verification.
...
- Результат:
  - final delivery checklist now explicitly covers OCR safety gate tests, extractor quality tests, focused `py_compile`, optional standalone OCR smoke with `--language rus+eng`, and storage/runtime cleanliness;
  - OCR safety gate is documented as misleading-success risk reduction, not an OCR quality guarantee;
  - RTF/PDF garbage detection is documented as bounded warning/degraded detection, not parser rewrite, OCR fallback or broad cleanup;
  - `.doc` dependency behavior and accepted deterministic ETL limitations remain accepted/preflight limitations, not backlog.
- Критерий завершения:

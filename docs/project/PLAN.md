...
- Реализация:
  - `scripts.inspect_document_structure`;
  - запуск: `conda run -n etl_env python -m scripts.inspect_document_structure --input-path "D:\path\file.docx"`;
  - input: one explicit file path;
  - processing only into isolated temporary workspace, default `.runtime_eval\inspect_document_structure_workspace`;
  - console summary by default, optional Markdown and JSON reports by explicit path;
  - report for manual review of metadata, processing info, sections, blocks, chunks, tables, images, warnings and artifacts;
  - dangerous workspace guard rejects production `storage` paths;
...
- Основной документ: `docs/project/OPERATION_GUIDE.md`.
- Оформлено:
  - короткую инструкцию;
...
- Основной документ: `docs/project/LANGUAGE_AUDIT.md`.
- Выполнено:
  - проведён bounded audit comments/docstrings/help/user-facing/docs text в project scope;
  - безопасно полированы selected FastAPI description и CLI/help/console strings;
  - API/JSON/CLI identifiers, имена тестов, technical symbols, stage names and command names сохранены.
- Scope:
...
- Основной документ: `docs/project/FINAL_DELIVERY_CHECKLIST.md`.
- Checklist включает:
  - full `pytest`;
  - demo smoke;
  - OCR smoke;
  - optional external validation smoke;
  - single-file inspector smoke после Stage 38.2;
  - UTF-8 sanity;
  - `git diff --check`;
  - storage/runtime pollution checks;
...
- Реализация:
  - bounded helper-level quality inspection for RTF text output and PDF paragraph fragments;
  - obvious RTF garbage is suppressed as degraded extraction with warning/metadata;
  - only CID-dominated PDF paragraph fragments are suppressed, while normal PDF text and table extraction remain unchanged.
- Вне scope:

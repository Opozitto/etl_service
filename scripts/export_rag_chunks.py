...
def main(argv: Sequence[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Read-only inspection/export RAG-ready chunks")
    parser.add_argument("--results-dir", default="storage/results", help="Директория с processed StructuredDocument JSON")
    parser.add_argument("--output-path", help="Опциональный путь для сохранения JSON export report")
    parser.add_argument("--max-documents", type=non_negative_int, help="Ограничить число processed JSON documents")
    parser.add_argument(
        "--max-chunks-per-document",
        type=non_negative_int,
        help="Ограничить exported chunks per document",
    )
    parser.add_argument("--include-text", action="store_true", help="Включить full chunk text в JSON output")
